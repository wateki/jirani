...
-- Add comments to explain tables
COMMENT ON TABLE public.delivery_options IS 'Stores delivery options that can be selected during checkout';
COMMENT ON TABLE public.deliveries IS 'Tracks all deliveries associated with orders';
COMMENT ON COLUMN public.deliveries.status IS 'Current status of the delivery: pending, processing, picked_up, in_transit, delivered, failed, canceled';
COMMENT ON COLUMN public.deliveries.current_location IS 'JSON object containing current location information of the delivery';
COMMENT ON COLUMN public.deliveries.delivery_timeline IS 'Array of status change events with timestamps';
